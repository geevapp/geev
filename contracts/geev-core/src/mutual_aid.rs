...
impl MutualAidContract {
    pub fn donate(env: Env, donor: Address, request_id: u64, amount: i128) {
        ...
        // Track donation for potential refunds
        ...
        env.storage()
            .persistent()
            .set(&donation_key, &(previous_donation + amount));
    ...
    pub fn cancel_request(env: Env, creator: Address, request_id: u64) {
        ...
        env.events()
            .publish((Symbol::new(&env, "RequestCancelled"), request_id), ());
    }
    ...
    pub fn claim_refund(env: Env, donor: Address, request_id: u64) {
        ...
        let amount: i128 = env
            ...
            .persistent()
            .get(&donation_key)
            .unwrap_or_else(|| panic_with_error!(&env, Error::InvalidDonationAmount));
