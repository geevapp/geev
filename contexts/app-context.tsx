...
function serializeState(state: AppState): string {
  const serializable = {
    ...
    user: state.user ? {
      ...state.user,
      joinedAt: state.user.joinedAt instanceof Date 
        ? state.user.joinedAt.toISOString() 
        : state.user.joinedAt,
      badges: state.user.badges.map(badge => ({
        ...badge,
        earnedAt: badge.earnedAt instanceof Date 
...
function deserializeState(stored: string): Partial<AppState> | null {
  try {
    ...
    return {
      ...
      // Restore user with proper Date objects
      user: parsed.user ? {
        ...parsed.user,
        joinedAt: new Date(parsed.user.joinedAt),
        badges: parsed.user.badges?.map((badge: Badge & { earnedAt: string }) => ({
          ...badge,
  ...
  const checkAndAwardBadges = useCallback(
    (userId: string, activityType: 'entry' | 'contribution') => {
      ...
      const availableBadges = [
        { min: 1, name: 'First Step', icon: '👣', color: 'bg-gray-100 text-gray-800' },
        { min: 5, name: 'Generous Giver', icon: '🎁', color: 'bg-blue-100 text-blue-800' },
        { min: 10, name: 'Community Hero', icon: '🦸', color: 'bg-purple-100 text-purple-800' },
        { min: 25, name: 'Legendary Giver', icon: '⭐', color: 'bg-yellow-100 text-yellow-800' },
        { min: 50, name: 'Giveaway Champion', icon: '🏆', color: 'bg-orange-100 text-orange-800' },
      ];
